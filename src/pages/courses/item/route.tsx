...
export default function CoursePartsRoute({course}: CourseProps): ReactElement {
  ...
  return parts == null ? <CircularProgress /> : (
    <Switch>
      <Route path={path}>
          <Redirect to={`${path}/${parts[0].slug}/`} />
      </Route>
...
export function CoursePartRoute({course, parts} : CoursePartRouteProps) : ReactElement {
    const { partId } = useParams<CoursePartParamTypes>();
    let { path } = useRouteMatch();
...
export function CoursePartItemRoute({course, part} : CoursePartProps) : ReactElement {
